...
class LiquidNeuron(nn.Module):
    def __init__(self, size: int, alpha_init: float = 0.5) -> None:
        super().__init__()
        self.W = nn.Linear(size, size)
        self.alpha = nn.Parameter(torch.ones(size) * alpha_init)
...
class MLP_LNN(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, width: int, depth: int) -> None:
        super().__init__()
        self.inp = nn.Linear(in_dim, width)
        self.hidden = nn.ModuleList(nn.Linear(width, width) for _ in range(max(depth - 1, 0)))
        self.liquid = nn.ModuleList(LiquidNeuron(width) for _ in range(depth))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = self.liquid[0](h)
        for layer, liquid in zip(self.hidden, self.liquid[1:]):
...
class RadialVortexPINN(nn.Module):
    def __init__(self, r_max: float, beta: float, dcoef: float, width: int, depth: int) -> None:
        super().__init__()
        self.r_max = float(r_max)
        self.net = MLP_LNN(1, 1, width, depth)

    def forward(self, r: torch.Tensor) -> torch.Tensor:
        z = 2.0 * r / self.r_max - 1.0
        return self.net(z)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="LNN-PINN for the 2D stationary Kerr vortex soliton."
    )
...
def train_pinn(args: argparse.Namespace, device: torch.device, dtype: torch.dtype) -> TrainResult:
    ...
    model = RadialVortexPINN(r_max, args.beta, args.D, args.width, args.depth).to(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        loss_robin = torch.mean((fr_b + lambda_tail * f_b) ** 2)
...
def write_metrics_file(
    ...
    with path.open("w", encoding="utf-8-sig") as f:
        f.write("# Kerr2D LNN-PINN run metrics\n")
